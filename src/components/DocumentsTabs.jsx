...
const documentTabsConfig = [
  ...
  { id: 'carteiraTrabalhoDigital', label: 'Carteira Trab. Digital', icon: FileText },
];
...
const DocumentsTabs = ({ employeeData, onDocumentChange, onFileChange }) => {
  ...
  const getDocumentStatus = (docType) => {
    ...
    return doc && doc.arquivos && doc.arquivos.length > 0 ? 'completed' : 'pending';
  };
    ...
    <Card className="document-card">
      ...
      <CardContent>
        <Tabs defaultValue="rg" className="space-y-4">
          <TabsList className="grid grid-cols-3 lg:grid-cols-5 xl:grid-cols-10 gap-1 h-auto flex-wrap">
